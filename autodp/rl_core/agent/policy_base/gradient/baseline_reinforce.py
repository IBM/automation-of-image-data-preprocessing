...
class BaselineReinforce(BaseAgent):
    """
    This class implements the baseline version of the REINFORCE policy.
    """
    def __init__(self):
        """
                ...
                for path in env.get_paths():
                    if path[-1][4]:
                        ...
                        for (t, p) in enumerate(path):
                            ...
                            baseline = sess.run(self._target_graph.get_value,
                                feed_dict={self._target_graph.get_instance: np.asarray([p[0]])})
                            advantage = sum_return - baseline

                            # Update our policy estimator
                            [_, err_policy] = sess.run(
