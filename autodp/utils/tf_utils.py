...
def declare_variable(name, shape, initializer, dev="cpu"):
    ...
    with tf.device(device):
        if shape is None:
            var = tf.get_variable(name=name, dtype=tf.float32, initializer=initializer)
        else:
...
def declare_variable_weight_decay(name, initializer, wd, shape=None, dev="cpu"):
    ...
    var = declare_variable(name=name, shape=shape, initializer=initializer, dev=dev)
...
def relu(x, alpha=0, max_value=None):
    """ Leaky rectified linear unit, with default values, it returns element-wise max(x, 0)."""
    if alpha != 0.:
...
def wrap_image(image, label_score):
    ...
    example = tf.train.Example(features=tf.train.Features(feature={"i": bytes_feature(image.tostring()),
