    ...
    def build_arch(self):
        ...
        with tf.variable_scope("conv1") as scope:
            kernel = declare_variable_weight_decay(initializer=kern_init,
                name="kernel", wd=0.0001, shape=[9, 9, 3, 8])
        ...
        with tf.variable_scope("conv2") as scope:
            kernel = declare_variable_weight_decay(initializer=kern_init,
                name="kernel", wd=0.0001, shape=[7, 7, 8, 16])
        ...
        with tf.variable_scope("conv3") as scope:
            kernel = declare_variable_weight_decay(initializer=kern_init,
                name="kernel", wd=0.0001, shape=[5, 5, 16, 32])
        ...
        with tf.variable_scope("conv4") as scope:
            kernel = declare_variable_weight_decay(initializer=kern_init,
                name="kernel", wd=0.0001, shape=[3, 3, 32, 64])
...
#            kernel = declare_variable_weight_decay(initializer=kern_init,
#                name="kernel", wd=0.0001, shape=[3, 3, 64, 250])
#            conv = tf.nn.conv2d(pool4, kernel, [1, 1, 1, 1], padding="SAME")
#            bias = declare_variable(name="bias", shape=[250],
#                                    initializer=bias_init)
#            #norm = tf.nn.lrn(input=relu(tf.nn.bias_add(conv, bias)),
#            #                 name="norm_pool")
#            pool5 = relu(tf.nn.bias_add(conv, bias))
        ...
        with tf.variable_scope("fc1") as scope:
            ...
            weights = declare_variable_weight_decay(initializer=xavi_init,
                name="weight", shape=[dim, 128], wd=0.0001)
        ...
        w_adv = declare_variable_weight_decay(initializer=xavi_init,
            name="w_adv", wd=0.0001, shape=[64, cf.num_action])
        w_val = declare_variable_weight_decay(initializer=xavi_init,
            name="w_val", shape=[64, 1], wd=0.0001)
